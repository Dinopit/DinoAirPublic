...
const LineChart = lazy(() => import('./line-chart').then(module => ({ default: module.LineChart })));
...
export function LazyChartLoader({ type, data, title, height = 300, ...props }: ChartLoaderProps) {
  const renderChart = () => {
    switch (type) {
      case 'line':
        return <LineChart data={data} title={title} height={height} {...props} />;
      case 'bar':
        return <BarChart data={data} title={title} height={height} {...props} />;
      case 'pie':
        return <PieChart data={data} title={title} height={height} {...props} />;
      case 'heatmap':
        return <Heatmap data={data} title={title} height={height} {...props} />;
      default:
